package com.codecriticon.ecommerce.web.rest;
...
import com.codecriticon.ecommerce.IntegrationTest;
...
class EcommerceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/ecommerce-kafka/publish?message=value-produce").with(csrf())).andExpect(status().isOk());
